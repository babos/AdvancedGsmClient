...
int16_t SIM7020HttpClient::createClientInstance() {
  ...
  if (scheme == SCHEME_HTTP) {
    ...
    if (this->server_ca == nullptr) {
      ...
      int16_t header_length = prefix_length + server_name_length + 1 + 1 + 1;
      ...
      // First chunk (header)
      this->modem.sendAT(GF("+CHTTPCREATEEXT="), is_more, ",", total_length, ",", header_length, ",\"", GSM_PREFIX_HTTPS, server_name, ",,,\"");
...
/*
      int16_t cert_chunk_size = max_chunk_size - header_length - server_ca_hex_length_string - 1;
      cert_section_end += (cert_chunk_size / 2);
      if (cert_section_end >= server_ca_length) {
        cert_section_end = server_ca_length;
        is_more = 0;
      }
      int16_t chunk_length = header_length + server_ca_hex_length_string + 1 + ((cert_section_end - cert_index) * 2);
      stream.print(GF("AT+CHTTPCREATEEXT="));
      stream.print(is_more);
      stream.print(',');
      stream.print(total_length);
      stream.print(',');
      stream.print(chunk_length);
      stream.print(",\"");
      // Initial content
      stream.print(GSM_PREFIX_HTTPS);
      stream.print(server_name);
      stream.print(",,,");
      stream.print("server_ca_hex_length_string");
      stream.print(",");
      while (cert_index < cert_section_end) {
        c = certificate[index];
        stream.printf("%2h", c);
      ...
      this->modem.sendAT(GF("+CHTTPCREATEEXT=1,"), total_length, ",", header_length, ",\"", GSM_PREFIX_HTTPS, server_name, ",,,\"");

      sendAT()
      stream.print(GF("AT+CHTTPCREATEEXT="));


      memcpy(buffer + index, GSM_PREFIX_HTTPS, prefix_length);
      index += prefix_length;
      memcpy(buffer + index, server_name, server_name_length);
      index += server_name_length;
      memcpy(buffer + index, comma, 1);
      index += 1;
      memcpy(buffer + index, server_ca_hex_length_string, server_ca_hex_length_string_length);
      index += server_ca_hex_length_string_length;
      memcpy(buffer + index, comma, 1);
      index += 1;

      int16_t server_ca_index = 0;
      while (server_ca_index < server_ca_length) {
        chunk_end += chunk_size;
        if (chunk_end >= length) {
          chunk_end = length;
          is_more = 0;
        }
        int16_t chunk_length = length - index;
